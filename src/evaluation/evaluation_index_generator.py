...
class SceneEvaluationIndexGenerator(LightningModule):
    generator: torch.Generator
    cfg: EvaluationIndexGeneratorCfg
    index: dict[str, IndexEntry | None]
    ...
    def generate_index(self, index, batch, extrinsics, intrinsics, scene, h, w, context_index, current_index):
        xy, _ = sample_image_grid((h, w), self.device)
        context_origins, context_directions = get_world_rays(
            rearrange(xy, "h w xy -> (h w) xy"),
            extrinsics[context_index],
        ...
        far = None #torch.zeros_like(context_origins[:,0]) + 2.0
        # Step away from context view until the minimum overlap threshold is met.
        valid_indices = False
        ...
        if valid_indices:
            # target = (context_index, current_index)
            self.index[f"{scene}_{index}"] = IndexEntry(
                context=(context_index, current_index),
                target=(context_index, current_index),
            )
            ...
            if self.cfg.save_previews:
                preview_path = self.cfg.output_path / "previews"
                preview_path.mkdir(exist_ok=True, parents=True)
                a = batch["target"]["image"][0, current_index]
                a = add_label(a, f"Overlap: {overlap_a * 100:.1f}%")
                b = batch["target"]["image"][0, context_index]
                b = add_label(b, f"Overlap: {overlap_b * 100:.1f}%")
                vis = add_border(add_border(hcat(a, b)), 1, 0)
                vis = add_label(vis, f"Distance: x frames")
                save_image(add_border(vis), preview_path / f"{scene}_{index}.png")
    ...
    def test_step(self, batch, batch_idx):
        ...
        scene = batch["scene"][0]
        scene_step = 0
        progress_bar = tqdm(total=self.cfg.num_pairs, desc=f"Generating index for {scene}")
        while True:
            context_indices = torch.randperm(v, generator=self.generator)[:2]
            valid_index = self.generate_index(scene_step, batch, extrinsics, intrinsics, scene, h, w, context_indices[0].item(), context_indices[1].item())
            if valid_index:
                scene_step += 1
                progress_bar.update(1)
            if scene_step >= self.cfg.num_pairs:
                break
