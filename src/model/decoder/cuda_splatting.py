...
from language_gs_rasterization import (
    GaussianRasterizationSettings,
    GaussianRasterizer,
)
...
def render_cuda(
    ...
    gaussian_features: Float[Tensor, "batch gaussian d_feat"] | None = None,
    ...
    if scale_invariant:
        scale = 1 / near # near is set to 1 in config
    ...
    view_matrix = rearrange(extrinsics.inverse(), "b i j -> b j i") # after inverse, put into gaussian should be w2c
    ...
    for i in range(b):
        ...
        settings = GaussianRasterizationSettings(
            ...
            debug=False,
            include_feature=True if gaussian_features is not None else False
        )
        rasterizer = GaussianRasterizer(settings)

        row, col = torch.triu_indices(3, 3)
        # import ipdb; ipdb.set_trace()
        image, feat, radii = rasterizer(
            ...
            language_feature_precomp=gaussian_features[i] if gaussian_features is not None else None,
        )
        all_images.append(image)
        all_radii.append(radii)
        all_viewspace_points.append(mean_gradients)
        all_feats.append(feat)
    
    return {
        "render": torch.stack(all_images),
        "viewspace_points": all_viewspace_points, # List[N,3] * bs
        "visibility_filter" : [r > 0 for r in all_radii], # List[N] * bs
        "radii": all_radii, # list [N]*bs
        "features": torch.stack(all_feats) if gaussian_features is not None else None
    }
...
def render_cuda_orthographic(
    ...
    for i in range(b):
        ...
        image, _, radii = rasterizer(
            ...
            opacities=gaussian_opacities[i, ..., None],
            cov3D_precomp=gaussian_covariances[i, :, row, col],
            language_feature_precomp=torch.zeros((gaussian_means[i].shape[0], 16), device=gaussian_means.device, dtype=gaussian_means.dtype),
        )
        all_images.append(image)
        all_radii.append(radii)
...
def render_depth_cuda(
    ...
    return result['render'].mean(dim=1)


def render_opacity_map_cuda(
    extrinsics: Float[Tensor, "batch 4 4"],
    intrinsics: Float[Tensor, "batch 3 3"],
    near: Float[Tensor, " batch"],
    far: Float[Tensor, " batch"],
    ...
    # Specify colors according to Gaussian depths.
    fake_color = gaussian_opacities
    # Render using opacity as color.
    b, _ = fake_color.shape
    result = render_cuda(
        extrinsics,
        intrinsics,
