...
class EncoderCostVolume(Encoder[EncoderCostVolumeCfg]):
    ...
    def __init__(self, cfg: EncoderCostVolumeCfg) -> None:
        ...
        ## gs predictor with feature
        gaussian_raw_channels = cfg.num_surfaces * (2 + self.gaussian_adapter.d_in + cfg.gs_feature_dim) if cfg.gs_feature_dim is not None else cfg.num_surfaces * (2 + self.gaussian_adapter.d_in) 
    ...
    def forward(
        ...
        if self.cfg.gs_feature_dim is not None:
            ...
            gs_features = rearrange(gs_features, "b v r srf c -> b (v r srf) c")
